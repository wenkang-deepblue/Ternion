...
async def test_optimizer_injects_text_tool_calls_instruction_for_non_openai_provider(
    ...
    def _fake_get_provider_for_role(_role: str) -> _CapturingTextToolsProvider:
        return provider
...
@pytest.mark.asyncio
async def test_optimizer_injects_scoped_ruff_verification_when_modified_python_files_present(
    monkeypatch: MonkeyPatch,
) -> None:
    provider = _CapturingTextToolsProvider()
    ...
    assert last_user is not None
    assert isinstance(last_user.content, str)
    assert "[MODIFIED FILES]" in last_user.content
    assert "[SCOPED VERIFICATION COMMANDS]" in last_user.content
    assert "python3 -m ruff check" in last_user.content
    assert "python3 -m ruff format --check" in last_user.content

    scoped_block = last_user.content.split("[SCOPED VERIFICATION COMMANDS]", 1)[1]
    assert "src/ternion/utils/evidence_chain.py" in scoped_block
    assert "README.md" not in scoped_block
